...
const Webrtc = (props) => {
  ...
  useEffect(() => {
    ...
    channel.current.onmessage = (e) => {
      switch (e.data.type) {
        case "setStore":
          // set folder hander in state
          setStore(e.data.data);
          break;
        case "getFolderPermission":
          handleGetPermission();
          break;
        case "offer":
          createOffer();
    ...
    socket.on("offerOrAnswer", (sdp) => {
      ...
      if (sdp.type === "offer") {
        ...
        sendMessage({
          type: "receviedOffer",
          data: {
            offerStatus: "Someone is Calling....",
            offerVisible: false,
          },
        });
      }
    ...
    pc.current.onconnectionstatechange = (e) => {
      setTimeout(() => {
        sendMessage({
          type: "status",
          data: {
            connStatus: pc.current.connectionState,
            channelStatus: sandChannel.current?.readyState,
          },
        });
      }, 100);
